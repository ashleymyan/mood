...
def analyze_feature_complexity(features: Union[torch.Tensor, np.ndarray], 
                             max_samples: int = DEFAULT_MAX_SAMPLES) -> dict:
    """
    Perform comprehensive analysis of feature complexity and dimensionality.
    
    Args:
        features: Input features to analyze
        max_samples: Maximum samples for estimation
        
    Returns:
        dict: Dictionary containing various dimensionality metrics and statistics
    """
    # Convert and flatten features
    if isinstance(features, torch.Tensor):
        numpy_features = features.cpu().detach().numpy()
    else:
        numpy_features = np.asarray(features)
    ...
    analysis = {
        ...
        'feature_mean': float(np.mean(flattened_features)),
        'feature_std': float(np.std(flattened_features)),
        'feature_range': float(np.ptp(flattened_features)),
    }
    
    try:
        # Estimate intrinsic dimension
        intrinsic_dim = estimate_intrinsic_dimension(features, max_samples)
        analysis['intrinsic_dimension'] = intrinsic_dim
        analysis['compression_ratio'] = n_features / intrinsic_dim if intrinsic_dim > 0 else np.inf
        analysis['dimensionality_reduction_potential'] = (n_features - intrinsic_dim) / n_features
        
    except Exception as e:
        analysis['intrinsic_dimension'] = None
...
get_intrinsic_dim = estimate_intrinsic_dimension